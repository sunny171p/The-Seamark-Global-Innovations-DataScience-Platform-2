...
One thing this didn't fix on its own: `01_data_cleaning.py` still doesn't know to skip a `test` or `voided` order by itself. That's a real gap, left here rather than pretending it isn't, and it's on the list to close before this listener gets pointed at anything that actually matters.
