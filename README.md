...
**A queryable warehouse.** `warehouse/build_duckdb.py` loads every CSV in `cleaned_data/` and `outputs/` into a single local DuckDB file. No server, no credentials, nothing to host, just a file that turns "open two CSVs and join them in pandas" into one SQL query. It's the same numbers, just queryable. Re-running it is genuinely idempotent -- same tables, same rows, every time, including cleaning up a table whose CSV no longer exists -- proven by `tests/test_warehouse.py` rather than just claimed. See `warehouse/README.md` for what that means in practice and why this is a full-refresh load rather than an incremental one, on purpose.
...
MIT — see `LICENSE`. Fork it, run it against your own store, adapt it for your own portfolio; the one condition is that the copyright notice in `LICENSE` stays intact in any copy.
